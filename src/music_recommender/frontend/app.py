...
def get_recommendations_from_audio(file_bytes, filename, top_n=10):
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE}/recommend/from-audio", files=files, params=params, timeout=30
        )
        if response.status_code == 200:
...
with st.sidebar:
    st.markdown(
        "<h2 class='gradient-text' style='text-align: center;'>🎵 Navigation</h2>",
...
if page == "🏠 Home":
    st.markdown(
        "<h1 class='main-title gradient-text'>🎵 SoundMatch AI</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p class='subtitle'>AI-Powered Music Discovery Through Audio Analysis</p>",
        unsafe_allow_html=True,
    )
    ...
    # API Status Section
    st.markdown(
        "<h2 class='gradient-text' style='text-align: center;'>🔌 System Status</h2>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        if status == "healthy":
            ...
            st.markdown(
                "<div class='status-badge status-degraded'>⚠️ Degraded</div>",
                unsafe_allow_html=True,
            )
            st.markdown(
    ...
    with col2:
        ...
        if models_loaded:
            st.markdown(
                "<div class='status-badge status-healthy'>🤖 Models Loaded</div>",
                unsafe_allow_html=True,
            )
            st.markdown(
            ...
            st.markdown(
                "<div class='status-badge status-offline'>🤖 Models Missing</div>",
                unsafe_allow_html=True,
            )
            st.markdown(
    ...
    with col3:
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        st.markdown(
            "<div class='status-badge' style='background: linear-gradient(90deg, #d4a5d4 0%, #f5a8c8 100%); color: white;'>💾 Database</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<p style='text-align: center; margin-top: 15px; color: #7b8794;'>Spotify tracks ready</p>",
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    # Features Grid
    st.markdown(
        "<h2 class='gradient-text' style='text-align: center;'>✨ Key Features</h2>",
        unsafe_allow_html=True,
    )
...
# Page 2: Upload & Recommend
    ...
    st.markdown(
        "<h1 class='main-title gradient-text'>🎧 Upload & Discover</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p class='subtitle'>Upload your audio file and let AI find your perfect matches</p>",
        unsafe_allow_html=True,
    )
    ...
    if status != "healthy" or not models_loaded:
        st.error(
            "⚠️ API is not available or models are not loaded. Please check the Home page for status."
        )
        st.stop()
    ...
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown(
        "<h3 style='color: #d4a5d4;'>📤 Upload Your Audio</h3>", unsafe_allow_html=True
    )
    ...
    if uploaded_file is not None:
        ...
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        st.markdown(
            "<h3 style='color: #f5a8c8;'>🎵 Preview</h3>", unsafe_allow_html=True
        )
        ...
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        st.markdown(
            "<h3 style='color: #a8d5ba;'>⚙️ Settings</h3>", unsafe_allow_html=True
        )
        top_n = st.slider(
        ...
        if st.button("🔮 Get Recommendations", use_container_width=True):
            with st.spinner("🎵 Analyzing audio and finding matches..."):
                ...
                result = get_recommendations_from_audio(
                    file_bytes, uploaded_file.name, top_n
                )
            ...
            if "error" in result:
                ...
                st.markdown("---")
                st.markdown(
                    "<h2 class='gradient-text'>🎵 Your Recommendations</h2>",
                    unsafe_allow_html=True,
                )
                ...
                for idx, song in enumerate(recommendations):
                    st.markdown(
                        "<div class='song-card fade-in'>", unsafe_allow_html=True
                    )
                    ...
                    with col1:
                        st.markdown(
                            f"<h2 style='color: #d4a5d4; text-align: center;'>{idx + 1}</h2>",
                            unsafe_allow_html=True,
                        )

                    with col2:
                        st.markdown(
                            f"<h3 style='color: #5a4a6a; margin: 0;'>{song['name']}</h3>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            f"<p style='margin: 5px 0; color: #7b8794;'>👤 <strong>{song['artists']}</strong></p>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            f"<p style='margin: 5px 0; color: #b39ddb;'>💿 {song['album']}</p>",
                            unsafe_allow_html=True,
                        )
                    ...
                    with col3:
                        ...
                        st.markdown(
                            "<p style='text-align: center; font-size: 0.8rem; color: #7b8794;'>Match</p>",
                            unsafe_allow_html=True,
                        )

                    with col4:
                        if st.button(
                            "⭐ Save", key=f"save_{idx}", use_container_width=True
                        ):
...
# Page 3: Favorites
    ...
    st.markdown(
        "<h1 class='main-title gradient-text'>⭐ Your Favorites</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p class='subtitle'>Your saved music collection</p>", unsafe_allow_html=True
    )
    ...
    if len(st.session_state["favorites"]) > 0:
        ...
        with col1:
            st.markdown(
                f"<h3 style='color: #d4a5d4;'>💾 {len(st.session_state['favorites'])} songs saved</h3>",
                unsafe_allow_html=True,
            )
        with col2:
        ...
        for idx, song in enumerate(st.session_state["favorites"]):
            ...
            with col1:
                st.markdown(
                    f"<h2 style='color: #f5a8c8; text-align: center;'>{idx + 1}</h2>",
                    unsafe_allow_html=True,
                )
            ...
            with col2:
                ...
                if "similarity_score" in song:
                    st.markdown(
                        f"<p style='margin: 5px 0; color: #a8d5ba;'>🎯 Match: {song['similarity_score'] * 100:.1f}%</p>",
                        unsafe_allow_html=True,
                    )

            with col3:
                if st.button(
                    "❌ Remove", key=f"remove_{idx}", use_container_width=True
                ):
...
# Page 4: About
    ...
    st.markdown(
        "<h1 class='main-title gradient-text'>ℹ️ About SoundMatch AI</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p class='subtitle'>Discover the technology behind the music</p>",
        unsafe_allow_html=True,
    )
    ...
    # Technology Section
    st.markdown(
        "<h2 class='gradient-text' style='text-align: center;'>🔬 Technology Stack</h2>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<h2 class='gradient-text' style='text-align: center;'>⚙️ How It Works</h2>",
        unsafe_allow_html=True,
    )
    ...
    # Features Section
    st.markdown(
        "<h2 class='gradient-text' style='text-align: center;'>✨ Predicted Features</h2>",
        unsafe_allow_html=True,
    )
